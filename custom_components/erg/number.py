...
class ErgJobNumber(NumberEntity):
    ...
    def __init__(
        ...
        self._attr_name = f"Erg {name_suffix} \u2014 {friendly_name(entity_id)}"
...
class ErgJobElapsedNumber(RestoreEntity, NumberEntity):
    ...
    def __init__(
        ...
        self._attr_name = f"Erg Elapsed Today \u2014 {friendly_name(entity_id)}"
...
def create_job_numbers(
    ...
    """Create number entities for a job's controllable attributes."""
    return [
        ErgJobNumber(
        ...
        ErgJobNumber(
            job_entity, coordinator, entry_id, entity_id,
            attr="min_ac_power", suffix="min_ac_power", name_suffix="Min AC Power",
            icon="mdi:flash-auto", native_min=0, native_max=100,
            native_step=0.1, unit="kW",
        ),
        ErgJobNumber(
            job_entity, coordinator, entry_id, entity_id,
        ...
        ErgJobNumber(
            job_entity, coordinator, entry_id, entity_id,
            attr="target_energy", suffix="target_energy", name_suffix="Target Energy",
            icon="mdi:battery-charging", native_min=0, native_max=1000,
            native_step=0.1, unit="kWh",
        ),
        ErgJobNumber(
            job_entity, coordinator, entry_id, entity_id,
            attr="min_energy", suffix="min_energy", name_suffix="Min Energy",
            icon="mdi:battery-charging-low", native_min=0, native_max=1000,
            native_step=0.1, unit="kWh",
        ),
        ErgJobNumber(
            job_entity, coordinator, entry_id, entity_id,
            attr="low_benefit", suffix="low_benefit", name_suffix="Low Benefit",
            icon="mdi:currency-usd-off", native_min=0, native_max=10000,
            native_step=0.01, unit="$",
        ),
