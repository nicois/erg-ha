...
UPDATE_JOB_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional("low_benefit"): vol.Coerce(float),
...
async def async_handle_check_health(hass: HomeAssistant, call: ServiceCall) -> None:
    """Handle erg.check_health service call."""
    _, entry_data = _find_entry_data(hass)
    coordinator = entry_data["coordinator"]
    ...
    hass.bus.async_fire(
        f"{DOMAIN}_health",
        {
            "last_solve_status": getattr(coordinator, "_last_solve_status", "unknown"),
            "last_solve_error": getattr(coordinator, "_last_solve_error", ""),
            "last_update": last_time.isoformat() if last_time else None,
            "active_jobs": len(entry_data.get("job_entities", {})),
        },
    )
...
def async_register_services(hass: HomeAssistant) -> None:
    ...
    hass.services.async_register(
        DOMAIN, SERVICE_CHECK_HEALTH, lambda call: async_handle_check_health(hass, call),
    )
...
def async_unregister_services(hass: HomeAssistant) -> None:
    ...
    hass.services.async_remove(DOMAIN, SERVICE_CHECK_HEALTH)
