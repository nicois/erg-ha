...
class TestAsyncGetActions:
    @pytest.mark.asyncio
    async def test_returns_ten_actions(self):
...
# ---------------------------------------------------------------------------
# set_ev_charging tests
# ---------------------------------------------------------------------------

class TestSetEvCharging:
    @pytest.mark.asyncio
    async def test_ev_charging_capabilities_schema(self):
        hass = MagicMock()
        caps = await async_get_action_capabilities(hass, {"type": "set_ev_charging"})
        schema = caps["extra_fields"]
        keys = {str(k) for k in schema.schema}
        assert "target_energy" in keys
        assert "min_energy" in keys
        assert "benefit" in keys
        assert "low_benefit" in keys

    @pytest.mark.asyncio
    async def test_ev_charging_updates_all_attributes(self):
        entity = ErgJobEntity(ENTRY_ID, {
            "entity_id": ENTITY_ID,
            "target_energy": 0.0,
            "min_energy": 0.0,
        ...
        assert attrs["target_energy"] == 50.0
        assert attrs["min_energy"] == 40.0
        assert attrs["benefit"] == 10.0
        assert attrs["low_benefit"] == 2.0
        entry_data["coordinator"].async_request_refresh.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_ev_charging_idempotent_overwrite(self):
        entity = ErgJobEntity(ENTRY_ID, {
            "entity_id": ENTITY_ID,
            "target_energy": 50.0,
            "min_energy": 40.0,
        ...
        with _patch_dr(registry):
            await async_call_action_from_config(hass, config1, {}, None)
        ...
        with _patch_dr(registry):
            await async_call_action_from_config(hass, config2, {}, None)

        attrs = entity.extra_state_attributes
        assert attrs["target_energy"] == 30.0
        assert attrs["min_energy"] == 20.0
        assert attrs["benefit"] == 5.0
        assert attrs["low_benefit"] == 1.0

    @pytest.mark.asyncio
    async def test_ev_charging_defaults_optional_fields(self):
        entity = ErgJobEntity(ENTRY_ID, {
            "entity_id": ENTITY_ID,
            "target_energy": 0.0,
            "benefit": 0.0,
        ...
        config = {
            ...
            "target_energy": 50.0,
            "benefit": 10.0,
            # min_energy and low_benefit omitted — should default to 0.0
        }
        with _patch_dr(registry):
            await async_call_action_from_config(hass, config, {}, None)
        ...
        assert attrs["target_energy"] == 50.0
        assert attrs["benefit"] == 10.0
        assert attrs["min_energy"] == 0.0
        assert attrs["low_benefit"] == 0.0
