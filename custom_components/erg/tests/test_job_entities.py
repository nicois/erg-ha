...
class TestJobEntityToDict:
    ...
    def test_ev_charging_attributes_round_trip(self):
        """min_energy and low_benefit survive from_job_dict → job_entity_to_dict."""
        job = {
            "entity_id": "switch.ev",
            "ac_power": 7.0,
            "min_ac_power": 1.4,
        ...
        result = job_entity_to_dict(entity)
        assert result["min_energy"] == 40.0
        assert result["low_benefit"] == 2.0
        assert result["target_energy"] == 50.0
        assert result["benefit"] == 10.0
