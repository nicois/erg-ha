...
class TestExpandRecurringJobs:
    ...
    def test_ev_fields_in_expanded_box(self):
        """min_energy and low_benefit flow through expand_recurring_jobs."""
        jobs = [
            {
                "entity_id": "switch.ev",
                "ac_power": 7.0,
        ...
        boxes = expand_recurring_jobs(jobs, start, end, UTC)
        assert len(boxes) == 1
        assert boxes[0]["min_energy"] == 40
        assert boxes[0]["low_benefit"] == 2
        assert boxes[0]["target_energy"] == 50
        assert boxes[0]["benefit"] == 10

    def test_ev_fields_in_oneshot_box(self):
        """min_energy and low_benefit flow through one-shot box expansion."""
        jobs = [
            {
                "entity_id": "switch.ev",
                "ac_power": 7.0,
                ...
                "enabled": True,
                "recurrence": None,
                "start": "2026-02-16T22:00:00+00:00",
                "finish": "2026-02-17T06:00:00+00:00",
                "maximum_duration": "6h",
                "minimum_duration": "0s",
                "minimum_burst": "0s",
            }
        ...
        start = datetime(2026, 2, 16, 0, 0, tzinfo=UTC)
        end = datetime(2026, 2, 18, 0, 0, tzinfo=UTC)
        boxes = expand_recurring_jobs(jobs, start, end, UTC)
        assert len(boxes) == 1
        assert boxes[0]["min_energy"] == 40
        assert boxes[0]["low_benefit"] == 2
    ...
    def test_depends_on_in_recurring_box(self):
        """depends_on flows through expand_recurring_jobs for recurring jobs."""
        jobs = [
            {
        ...
        boxes = expand_recurring_jobs(jobs, start, end, UTC)
        assert len(boxes) == 1
        assert boxes[0]["depends_on"] == "switch.washer"

    def test_depends_on_in_oneshot_box(self):
        """depends_on flows through expand_recurring_jobs for one-shot jobs."""
        jobs = [
            {
                "entity_id": "switch.dryer",
                "ac_power": 3.0,
                ...
                "finish": "2026-02-16T17:00:00+00:00",
                "maximum_duration": "1h",
                "minimum_duration": "0s",
                "minimum_burst": "0s",
            }
    ...
    def test_depends_on_defaults_to_empty(self):
        """depends_on defaults to empty string when not specified."""
        jobs = [
            {
                "entity_id": "switch.pump",
                "ac_power": 1.0,
