...
class TestSolveStatusSensor:
    """Tests for the solve_status sensor."""

    def test_returns_unknown_by_default(self):
        coordinator = _make_coordinator(data={"some": "data"})
        # Delete the auto-created mock attribute so getattr falls back to default
        del coordinator._last_solve_status
        entry = _make_entry()
        desc = next(d for d in GLOBAL_SENSORS if d.key == "solve_status")
        sensor = ErgGlobalSensor(coordinator, entry, desc)
        assert sensor.native_value == "unknown"

    def test_returns_ok_after_success(self):
        coordinator = _make_coordinator(data={"some": "data"})
        coordinator._last_solve_status = "ok"
        coordinator._last_solve_error = ""
        entry = _make_entry()
        desc = next(d for d in GLOBAL_SENSORS if d.key == "solve_status")
        sensor = ErgGlobalSensor(coordinator, entry, desc)
    ...
    def test_returns_error_status(self):
        coordinator = _make_coordinator(data={"some": "data"})
        coordinator._last_solve_status = "infeasible"
        coordinator._last_solve_error = "forced jobs exceed grid limit"
        entry = _make_entry()
        desc = next(d for d in GLOBAL_SENSORS if d.key == "solve_status")
        sensor = ErgGlobalSensor(coordinator, entry, desc)
        assert sensor.native_value == "infeasible"

    def test_extra_attributes_include_error(self):
        coordinator = _make_coordinator(data={"some": "data"})
        coordinator._last_solve_status = "error"
        coordinator._last_solve_error = "connection refused"
        entry = _make_entry()
        desc = next(d for d in GLOBAL_SENSORS if d.key == "solve_status")
        sensor = ErgGlobalSensor(coordinator, entry, desc)
        attrs = sensor.extra_state_attributes
        assert attrs == {"error": "connection refused"}

    def test_extra_attributes_none_when_no_error(self):
        coordinator = _make_coordinator(data={"some": "data"})
        coordinator._last_solve_status = "ok"
        coordinator._last_solve_error = ""
        entry = _make_entry()
...
class TestAsyncSetupEntry:
    ...
    async def test_creates_global_and_per_job_entities_from_migration(self, sample_schedule_data):
        ...
        # 11 global + 1 job entity (pool_pump, __solar__ filtered) + 3 per-job sensors
        assert len(added) == 15
    ...
    async def test_creates_sensors_from_existing_job_entities(self, sample_schedule_data):
        ...
        # 11 global + 1 job entity + 3 per-job sensors
        assert len(added) == 15
    ...
    async def test_filters_dunder_entities(self, sample_schedule_data):
        ...
        # Only 11 global sensors, no per-job (no job entities)
        assert len(added) == 11
