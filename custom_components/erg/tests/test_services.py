...
class TestCheckHealth:
    @pytest.mark.asyncio
    async def test_fires_health_event(self):
        coordinator = MagicMock()
        coordinator.async_request_refresh = AsyncMock()
        ...
        entry_data["job_entities"] = {"switch.pump": MagicMock()}
        hass = _make_hass(entry_data)
        call = _make_call({})

        await async_handle_check_health(hass, call)

        hass.bus.async_fire.assert_called_once()
        event_name, event_data = hass.bus.async_fire.call_args[0]
        assert event_name == "erg_health"
        assert event_data["last_solve_status"] == "ok"
        assert event_data["last_solve_error"] == ""
        assert event_data["active_jobs"] == 1
